...
#' @export
#'
searchForCategoryGroups <- function(search_conditions, order_by = "identifier", sort_by = "asc", result_limit = NA, uuids_only = F) {
  # Search for the entity on the path with the provided search conditions
  return(searchForEntity(path = "category_groups", conditions = search_conditions, order_by = order_by, sort_by = sort_by, result_limit = result_limit, uuids_only = uuids_only))
}