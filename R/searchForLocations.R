#'Function to search for locations that fulfill pre-specified search conditions over the Search API
#'
...
#'
#' @examples
#' searchForLocations(rbind(searchCondition1,searchCondition2))
#'
#' @export
#'
searchForLocations <- function(search_conditions, order_by = "identifier", sort_by = "asc", result_limit = NA, uuids_only = F, precise = T, iteration = 1000L) {
  # Search for the entity on the path with the provided search conditions
  return(searchForEntity(path = "locations", conditions = search_conditions, order_by = order_by, sort_by = sort_by, result_limit = result_limit, uuids_only = uuids_only, precise = precise, iter = iteration))
}