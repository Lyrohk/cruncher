...
#'
#' @examples
#' lookUpCategoryGroups(c("	f9b14a15-5517-8f38-0562-729ebb54dfdb", "e8804a82-8603-d2e1-c3b5-d241dd8030b9"))
#' lookUpCategoryGroups(list("	f9b14a15-5517-8f38-0562-729ebb54dfdb", "e8804a82-8603-d2e1-c3b5-d241dd8030b9"))
#'
#' @import dplyr
#' @export
#'
...
lookUpCategoryGroups <- function(category_groups, please_parse = TRUE) {
  ...
  if (!class(category_groups) %in% c("list", "character")) {
  ...
  if (please_parse) {
    ...
    df <- do.call(rbind.data.frame, lapply(X = category_groups, FUN = lookUpPerson))
    ...
    return(do.call(list, lapply(X = persons, FUN = category_groups, please_parse = FALSE)))
  }
}