#'Function to search for organizations that fulfill pre-specified search conditions over the Search API
#'
...
#'@param result_limit (Optional) numeric limit of the number of results you'd like to have. By default, all results will be returned to you. Only accurate to 1000 level, no smaller iterations.
...
#'
#' @examples
#' searchForOrganizations(rbind(searchCondition1,searchCondition2))
#'
#' @export
#'
searchForOrganizations <- function(search_conditions, order_by = "identifier", sort_by = "asc", result_limit = NA, uuids_only = F, precise = T, iteration = 1000L) {
  # Search for the entity on the path with the provided search conditions
  return(searchForEntity(path = "organizations", conditions = search_conditions, order_by = order_by, sort_by = sort_by, result_limit = result_limit, uuids_only = uuids_only, precise = precise, iter = iteration))
}