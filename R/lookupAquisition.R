#' Function to lookup a single acquisition over the Entity Lookup API Endpoint
#'
#'@param id UUID or permalink of the acquisition you wish to look up
#'@param parse TRUE or FALSE. By default TRUE. If set to FALSE, it will return the data directly from the JSON, if set to TRUE, it will parse it into a data.frame object
#'@return either a data.frame (if parse = TRUE) or a list (if parse = FALSE)
#'
#' @author Layla Rohkohl, \email{byehity@gmail.com}
#'
#' @examples
#' lookUpAquisition("uuid")
#'
#' @import httr
#' @import jsonlite
#' @export
#'
# Call the function to get you information about an acquisition
lookUpAquisition <- function(id, please_parse = TRUE)  {
  
  # Check that API_KEY exists
  if (!exists("API_KEY")) {
    stop("Please set a valid user key to API_KEY as an environmental variable or for use setAPIKey() to do it for you.")
  }
  
  # Check that id has been specified 
  if (missing(id)) {
    stop("Please enter a uuid or permalink for the person you wish to look up. You can find them manually from the browser or csv files.")
  }
  
  # Entity id check e.g. to lower, trimming whitespace, and replacing spaces with -
  id <- entityIdCheck(id)
  
  # Create the path
  url <- paste0("https://api.crunchbase.com/api/v4/entities/acquisitions/", id, "?card_ids=fields&user_key=", API_KEY)
  
  # Make http GET request
  response <- RETRY(verb = "GET", url = url) # Could use GET but someone may apply it with a list
  
  # Check if we get valid data, if not return error core
  if (response$status_code == 200) {
    data <- fromJSON(rawToChar(response$content))
    # Check if parsing is wanted
    if (please_parse) {
      
      # Parse acquisition to return a dataframe
      # Simple parsing
      uuid <- simpleParse(data[["properties"]][["identifier"]][["uuid"]])
      name <- simpleParse(data[["properties"]][["identifier"]][["value"]])
      short_description <- simpleParse(data[["cards"]][["fields"]][["short_description"]])
      acquirer_short_description <- simpleParse(data[["cards"]][["fields"]][["acquirer_short_description"]])
      acquiree_short_description <- simpleParse(data[["cards"]][["fields"]][["acquiree_short_description"]])
      status <- simpleParse(data[["cards"]][["fields"]][["status"]])
      rank_acquisition <- simpleParse(data[["cards"]][["fields"]][["rank_acquisition"]])
      acquiree_revenue_range <- simpleParse(data[["cards"]][["fields"]][["acquiree_revenue_range"]])
      acquirer_revenue_range <- simpleParse(data[["cards"]][["fields"]][["acquirer_revenue_range"]]) 
      acquirer_funding_stage <- simpleParse(data[["cards"]][["fields"]][["acquirer_funding_stage"]])
      acquisition_type <- simpleParse(data[["cards"]][["fields"]][["acquisition_type"]])
      acquiree_num_funding_rounds <- simpleParse(data[["cards"]][["fields"]][["acquiree_num_funding_rounds"]])
      acquirer_num_funding_rounds <- simpleParse(data[["cards"]][["fields"]][["acquirer_num_funding_rounds"]])
      acquiree_last_funding_type <- simpleParse(data[["cards"]][["fields"]][["acquiree_last_funding_type"]])
      created_at <- simpleParse(data[["cards"]][["fields"]][["created_at"]])
      terms <- simpleParse(data[["cards"]][["fields"]][["terms"]])
      updated_at <- simpleParse(data[["cards"]][["fields"]][["updated_at"]])
      
      # List parsing
      announced_on <- listParse(data[["cards"]][["fields"]][["announced_on"]])
      acquirer_identifier <- listParse(data[["cards"]][["fields"]][["acquirer_identifier"]])
      acquiree_identifier <- listParse(data[["cards"]][["fields"]][["acquiree_identifier"]])
      acquirer_categories <- listParse(data[["cards"]][["fields"]][["acquirer_categories"]])
      acquiree_categories <- listParse(data[["cards"]][["fields"]][["acquiree_categories"]])
      acquirer_locations <- listParse(data[["cards"]][["fields"]][["acquirer_locations"]])
      acquiree_locations <- listParse(data[["cards"]][["fields"]][["acquiree_locations"]])
      # Currency list parsing
      acquirer_funding_total <- currencyListParse(data[["cards"]][["fields"]][["acquirer_funding_total"]])
      acquiree_funding_total <- currencyListParse(data[["cards"]][["fields"]][["acquiree_funding_total"]])
      price <- currencyListParse(data[["cards"]][["fields"]][["price"]])
      
      # Put into one dateframe
      df <- data.frame(cbind(  uuid ,
                               name,
                               short_description ,
                               acquirer_short_description ,
                               acquiree_short_description,
                               status ,
                               rank_acquisition ,
                               acquiree_revenue_range ,
                               acquirer_revenue_range ,
                               acquirer_funding_stage ,
                               acquisition_type ,
                               acquiree_num_funding_rounds ,
                               acquirer_num_funding_rounds ,
                               acquiree_last_funding_type ,
                               created_at ,
                               terms ,
                               updated_at, # Simple parsing done
                               announced_on ,
                               acquiree_identifier ,
                               acquirer_categories ,
                               acquiree_categories ,
                               acquirer_locations ,
                               acquiree_locations ,
                               # Currency list parsing
                               acquirer_funding_total,
                               acquiree_funding_total ,
                               price 
                               
      ))
      
      # Adjust classes
      # as numeric
      df$acquirer_funding_total <- as.numeric(df$acquirer_funding_total)
      df$acquiree_funding_total <- as.numeric(df$acquiree_funding_total)
      df$price <- as.numeric(df$price)
      df$acquiree_num_funding_rounds <- as.numeric(df$acquiree_num_funding_rounds)
      df$acquirer_num_funding_rounds <- as.numeric(df$acquirer_num_funding_rounds)
      # as date
      df$announced_on <- as.Date(df$announced_on)
      df$created_at <- as.Date(df$created_at)
      df$updated_at <- as.Date(df$updated_at)
      
      # Return dataframe
      df
      
    } else {
      # Return data
      return(data)
    }
  } else {
    # Print error code
    print(response$status_code)
  }
}