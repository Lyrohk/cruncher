...
parseOrganization <- function(data) {
  ...
  name <- listParse(data[["identifier"]])
  company_type <- simpleParse(data[["company_type"]])
  legal_name <- simpleParse(data[["legal_name"]])
  num_employees_enum <- simpleParse(data[["num_employees_enum"]])
  operating_status <- simpleParse(data[["operating_status"]])
  status <- simpleParse(data[["status"]])
  revenue_range <- simpleParse(data[["revenue_range"]])
  ...
  ipo_status <- simpleParse(data[["ipo_status"]])
  image_url <- simpleParse(data[["image_url"]])
  last_equity_funding_type <- simpleParse(data[["last_equity_funding_type"]])
  hub_tags <- simpleParse(data[["hub_tags"]])
  valuation_date <- simpleParse(data[["valuation_date"]])
  last_funding_type <- simpleParse(data[["last_funding_type"]])
  ...
  funding_stage <- simpleParse(data[["funding_stage"]])
  # Numeric elements
  ...
  num_enrollments <- simpleParse(data[["num_enrollments"]])
  ...
  school_method <- simpleParse(data[["school_method"]])
  school_program <- simpleParse(data[["school_program"]])
  school_type <- simpleParse(data[["school_type"]])
