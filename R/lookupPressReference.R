#' Function to lookup a single press reference over the Entity Lookup API Endpoint
#'
#'@param id UUID or permalink of the press reference you wish to look up
...
#'
#' @examples
#' lookUpPressReference("uuid")
#'
#' @import httr
#' @import jsonlite
#' @export
#'
# Call the function to get you information about an press reference
lookUpPressReference <- function(id, please_parse = TRUE)  {
  ...
  # Create the path
  url <- paste0("https://api.crunchbase.com/api/v4/entities/press_references/", id, "?card_ids=fields&user_key=", API_KEY)
  ...
  if (response$status_code == 200) {
    ...
    if (please_parse) {
      ...
      publisher <- simpleParse(data[["cards"]][["fields"]][["publisher"]])
      posted_on <- simpleParse(data[["cards"]][["fields"]][["posted_on"]])
      created_at <- simpleParse(data[["cards"]][["fields"]][["created_at"]])
      updated_at <- simpleParse(data[["cards"]][["fields"]][["updated_at"]])
      
      # List parsing
      url <- listParse(data[["cards"]][["fields"]][["url"]])
      activity_entities <- listParse(data[["cards"]][["fields"]][["activity_entities"]])
